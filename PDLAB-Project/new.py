...
def mainGame():
    ...
    while True:
        ...
        # Check if the character is above or below the pipes and adjust position
        if (
            charx + current_char.get_width() > pipe1x
            and charx < pipe1x + game_sprites["pipe"].get_width()
            and chary < pipe1y + game_sprites["pipe"].get_height()
        ):
            charx = pipe1x - current_char.get_width()

        if (
            charx + current_char.get_width() > pipe2x
            and charx < pipe2x + game_sprites["pipe"].get_width()
            and chary < pipe2y + game_sprites["pipe"].get_height()
        ):
            charx = pipe2x - current_char.get_width()
